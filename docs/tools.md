...
Browser-saved cloud models can opt into tool calling per selected model from `Settings -> Cloud Providers`. When that toggle is on, the app uses a generic JSON prompt profile with `{"name":"tool_name","parameters":{...}}`, and the detector also accepts `arguments` if the remote model emits that key instead.
