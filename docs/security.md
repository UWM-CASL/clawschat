...
- Proxy fallback is skipped for same-origin requests and attempts to send local/private-network targets through a remote proxy. General browser-networked features still avoid proxying explicit authorization headers, but user-configured OpenAI-compatible cloud providers may route API-key-bearing `/models` and `/chat/completions` requests through the saved proxy when the provider is not directly browser-readable.
...
- OpenAI-compatible cloud providers are tested with an authenticated `GET /models` call before save. If that initial model-list request appears CORS-blocked and the user has a saved proxy, the app retries through the proxy and stores that provider as proxy-required so later authenticated `/chat/completions` requests use the proxy immediately. Providers that pass direct model loading still use direct browser requests first and retry through the saved proxy only after a likely CORS failure.
- Cloud-provider `.cloud-pro.json` exports intentionally omit API keys. Import requires the user to enter a key, validates it with the same authenticated `/models` inspection, and restores exported model settings only for models returned by that validation response.
