...
- `models[].languageSupport`: optional user-facing language badge metadata for the pre-chat picker
...
- The pre-chat picker presents each model as a single-select card with icon-only ability badges, language support, short-term memory, and a rough word estimate (`tokens * 0.75`).
