...
If a generation request stops emitting worker activity for 90 seconds, the main-thread engine client terminates that worker and returns a recoverable timeout so the next request can reinitialize cleanly. CPU/WASM local-model generation uses a 300-second timeout only until the first printable token arrives, because Llama 3.2 CPU prefill can be silent for several minutes on first inference; after streaming starts, the normal 90-second inactivity timeout applies.
