...
- Models with `multimodalGeneration: true` can still initialize through the text-generation path when the current prompt contains no image/audio/video inputs; the worker reinitializes into the processor/model path only when media is actually present. Models may opt into staying on the multimodal runtime for text-only turns when the lighter path is not stable enough in-browser; the bundled Gemma 4 entry currently does this.
