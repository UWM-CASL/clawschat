...
On the `wllama` path, `maximum context size` is part of model initialization, so the next request reinitializes the worker when that setting changes; before each generation, the worker also tokenizes the formatted prompt and trims the oldest prompt tokens to stay within the configured budget.
