...
- The app now keeps a browser-local semantic memory layer derived from prior user statements and agent summary nodes; prompt assembly retrieves a compact set of relevant memories for the current user turn from that same conversation only instead of depending on the model to remember every durable fact or preference from raw transcript alone.
