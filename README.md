...
- `onnx-community/Qwen3-1.7B-ONNX`
  - Uses the Transformers.js worker path in this app.
  - Uses `q4` on WebGPU and `uint8` on CPU in this app.
  - Stays text-only in the current worker path.
  - Uses Qwen3's `<think>...</think>` reasoning tags, runtime `enable_thinking`, and the app's XML tool-call format support.
