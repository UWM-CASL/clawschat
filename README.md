...
- When one or more configured MCP servers are enabled and have enabled commands, that tool-calling block also adds an `MCP Servers List` section for the enabled server inventory plus an `MCP Server Instructions` section for the discovery/call syntax of `list_mcp_server_commands` and `call_mcp_server_command`.
- Those MCP helper tools run through the same tool harness as built-in tools, but they are listed only under the MCP prompt sections and are not user-toggled in `Settings -> Tools`.
